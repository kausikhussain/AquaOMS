...
function checkProductStockAndNotify(db, product) {
  const threshold = db.settings?.lowStockThreshold ?? 20;
  if (product.stock < threshold) {
    if (!db.notifications) {
      db.notifications = [];
    }
    const alreadyNotified = db.notifications.some((n) => n.type === "low_stock" && n.productId === product.id && n.unread);
    if (!alreadyNotified) {
...
var AUTHORIZED_RETAILERS = ["papan@amritdhara.com", "retailer@amritdhara.com", "admin@amritdhara.com"];
app.post("/api/auth/register", (req, res) => {
  ...
  const existingUser = db.users.find((u) => u.email.toLowerCase().trim() === emailLower);
  if (existingUser) {
  ...
  if (role === "customer") {
    const existingCustomer = db.customers.find((c) => c.email.toLowerCase().trim() === emailLower);
    if (existingCustomer) {
...
app.post("/api/auth/login", (req, res) => {
  ...
  const emailLower = email.toLowerCase().trim();
  const user = db.users.find((u) => u.email.toLowerCase().trim() === emailLower && u.password === password);
  if (!user) {
...
app.post("/api/orders", (req, res) => {
  ...
  if (newOrder.paymentStatus === "Outstanding" || newOrder.paymentMethod === "Pay Later") {
    const custIdx2 = db.customers.findIndex((c) => c.id === newOrder.customerId);
    if (custIdx2 !== -1) {
      db.customers[custIdx2].outstandingBalance += newOrder.total;
    }
  }
  const custIdx = db.customers.findIndex((c) => c.id === newOrder.customerId);
  if (custIdx !== -1) {
    db.customers[custIdx].monthlySpending += newOrder.total;
    db.customers[custIdx].totalPurchases += newOrder.products.reduce((acc, p) => acc + (p.quantity || 1), 0);
  }
  db.payments.push({
    id: `pay-${Date.now()}`,
  ...
  newOrder.products.forEach((p) => {
    const pIdx = db.products.findIndex((prod) => prod.id === p.productId);
    if (pIdx !== -1) {
...
app.get("/api/subscriptions", (req, res) => {
  ...
  const { customerId } = req.query;
  if (customerId) {
    return res.json(db.subscriptions.filter((s) => s.customerId === customerId));
  }
...
app.post("/api/payments/settle-due", (req, res) => {
  ...
  const custIdx = db.customers.findIndex((c) => c.id === customerId);
  if (custIdx !== -1) {
...
app.get("/api/analytics", (req, res) => {
  const db = readDB();
  const totalRevenue = db.payments.filter((p) => p.status === "Completed").reduce((sum, p) => sum + p.amount, 0);
  const outstandingBalance = db.customers.reduce((sum, c) => sum + c.outstandingBalance, 0);
  const pendingDeliveries = db.orders.filter((o) => o.status !== "Delivered" && o.status !== "Cancelled").length;
  const lowStockThreshold = db.settings?.lowStockThreshold ?? 20;
  const lowStockProducts = db.products.filter((p) => p.stock < lowStockThreshold).length;
  ...
  const outstandingTrends = db.customers.map((c) => ({
    name: c.name,
  ...
  res.json({
    ...
    totalBottlesSold: db.customers.reduce((sum, c) => sum + c.totalPurchases, 0),
    revenueChart,
    outstandingTrends,
...
app.post("/api/reviews", (req, res) => {
  ...
  const pIdx = db.products.findIndex((p) => p.id === newReview.productId);
  if (pIdx !== -1) {
    const prodReviews = db.reviews.filter((r) => r.productId === newReview.productId);
    const totalRating = prodReviews.reduce((acc, r) => acc + r.rating, 0);
...
app.post("/api/ai/insights", async (req, res) => {
  ...
  const customerSubs = db.subscriptions.filter((s) => s.customerId === customer.id);
  if (!ai) {
    return res.json({
  ...
  try {
    ...
    `;
    const generateWithRetry = async (client, params, retries = 3, initialDelay = 1e3) => {
      ...
      for (let i = 0; i < retries; i++) {
        try {
          ...
          const isTransient = errMessage.includes("503") || errMessage.includes("demand") || errMessage.includes("unavailable") || errMessage.includes("429") || errMessage.includes("limit") || errMessage.includes("overloaded");
